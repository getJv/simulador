  ...
  <v-row justify="center" align="center">
    <v-col cols="12" sm="8">
      <v-card class="mx-auto mt-5" color="green green-ligthen-3" dark max-width="400">
        <v-card-title>
          <span class="title font-weight-light text-uppercase">Total dos custos fixos</span>
        </v-card-title>

        <v-card-text class="headline font-weight-bold text-center">
          <v-divider class="pa-2"></v-divider>
          {{ cg_hfh_cf_total | currency }}
        </v-card-text>
      </v-card>
    </v-col>
    <v-col cols="12" sm="10">
...
export default {
  name: "CustoFixo",
  computed: {
    ...mapGetters(["cg_hfh_cf_total", "cg_hfh_custos_fixos"])
  },
  data() {
    return {
      headers: [
