        ...
        >
          <template v-slot:top>
            <v-toolbar flat color="white">
              <v-toolbar-title>Custos variáveis</v-toolbar-title>
    ...
    >
      <v-card-title>
        <span class="title font-weight-light text-uppercase "
          >Total dos Custos Variáveis</span
        >
      </v-card-title>

      <v-card-text class="headline font-weight-bold text-center">
        <v-divider class="pa-2"></v-divider>
        {{ cv_total_custo_variavel | currency }}</v-card-text
      >
    </v-card>
  </div>
</template>
...
export default {
  name: 'CustoVariavel',
  mounted() {
    this.itens = this.custo_variavel
  },
  methods: {
    updateValue(e) {
      this.$store.commit('custoVariavelByFieldName', {
        formula: e.target.id,
    ...
    editItem(item) {
      this.editedIndex = this.getAllCustosVariaveis.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = true
    },

    deleteItem(item) {
      const index = this.getAllCustosVariaveis.indexOf(item)
      confirm('Deseja realmente excluir este item?') &&
        this.$store.commit('removeCustoVariavel', index)
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.$store.commit('addCustoVariavel', {
  ...
  computed: {
    ...mapGetters(['cv_total_custo_variavel', 'custo_variavel']),
    formTitle() {
