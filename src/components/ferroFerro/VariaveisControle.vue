    ...
    <v-col cols="12" sm="3">
      ...
      >
        ...
        <v-card-text class="headline font-weight-bold text-center">
          ...
          {{
            cg_ff_getVar("cg_ff_variaveis_de_ctrl", "cg_ff_ctrl_tarifa_limpa")
              .valor | currency
          }}
        </v-card-text>
      </v-card>
    ...
    <v-col cols="12" sm="3">
      ...
      >
        ...
        <v-card-text class="headline font-weight-bold text-center">
          <v-divider class="pa-2"></v-divider>
          {{
            cg_ff_getVar("cg_ff_variaveis_de_ctrl", "cg_ff_ctrl_lucro").valor
              | currency
          }}
        </v-card-text>
      </v-card>
    ...
    <v-col cols="12" sm="3">
      ...
      >
        ...
        <v-card-text class="headline font-weight-bold text-center">
          <v-divider class="pa-2"></v-divider>
          {{
            cg_ff_getVar("cg_ff_variaveis_de_ctrl", "cg_ff_ctrl_tributos")
              .valor | currency
          }}
        </v-card-text>
      </v-card>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-data-table
            ...
            hide-default-footer
            :headers="headers"
            :items="cg_ff_variaveis_de_ctrl"
          >
            <template v-slot:item.valor="{ item }">
              <span v-if="item.filters.find(item => item == 'currency')">{{
                item.valor | currency
              }}</span>
              <span v-else-if="item.filters.find(item => item == 'percent')">{{
                item.valor | percent
              }}</span>
...
export default {
  ...
  computed: {
    ...mapGetters(["cg_ff_getVar", "cg_ff_variaveis_de_ctrl"])
  },
  filters: {
