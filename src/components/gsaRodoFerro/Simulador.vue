<template>
  <v-row justify="center" align="center">
    ...
    <v-col
      cols="12"
      md="4"
      v-for="v in gsa_rf_variaveis_de_entrada"
      :key="v.id"
    >
      <v-text-field
        outlined
            ...
            <v-card-actions>
              <v-dialog v-model="dialog" width="1500">
                <template v-slot:activator="{ on }">
                  <v-btn outlined block class="text-center" v-on="on"
...
<script>
import Tabs from "@/components/gsaRodoFerro/Tabs";
...
export default {
  ...
  methods: {
    ...
    iniciar_variaveis_de_entrada() {
      var ordem = this.gsa_rf_variaveis_de_entrada.sort(
        (a, b) => a.ordem - b.ordem
      );
      ordem.forEach((item) => {
    ...
    iniciar_variaveis_de_controle() {
      var ordem = this.gsa_rf_variaveis_de_ctrl.sort(
        (a, b) => a.ordem - b.ordem
      );
      ordem.forEach((item) => {
        this.$store.dispatch(item.formula);
    ...
    iniciar_custos_variaveis() {
      var ordem = this.gsa_rf_custos_variaveis.sort(
        (a, b) => a.ordem - b.ordem
      );
      ordem.forEach((item) => {
        this.$store.dispatch(item.formula);
    ...
    iniciar_custos_fixos() {
      var ordem = this.gsa_rf_custos_fixos.sort((a, b) => a.ordem - b.ordem);
      ordem.forEach((item) => {
        this.$store.dispatch(item.formula);
        //console.log(item.ordem + ' - ' + item.formula + ' Iniciado')
      });
